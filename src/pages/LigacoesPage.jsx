...
export default function LigacoesPage() {
  ...
  const [calMetric, setCalMetric]             = useState('calls') // 'calls' | 'appointments'
  ...
  const maxVal = useMemo(() => {
    const vals = calDays
      ...
      .map(d => {
        ...
        return l ? (calMetric === 'calls' ? l.calls : l.appointments) : 0
      })
  ...
  const accentRGB  = calMetric === 'calls' ? '96,165,250' : '167,139,250'
  const accentHex  = calMetric === 'calls' ? '#60A5FA'    : '#A78BFA'
    ...
    <div className="animate-in" style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
      ...
      <div className="rounded-2xl" style={{ background: '#161616', border: '1px solid #252525', padding: '20px' }}>
        ...
        <div className="flex gap-2" style={{ marginBottom: '14px' }}>
          {[
            { key: 'calls',        label: 'Ligações',  Icon: Phone,  color: '#60A5FA', rgba: 'rgba(96,165,250,' },
            { key: 'appointments', label: 'Marcações', Icon: MapPin, color: '#A78BFA', rgba: 'rgba(167,139,250,' },
          ].map(({ key, label, Icon, color, rgba }) => (
            <button key={key} onClick={() => setCalMetric(key)}
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-semibold"
              style={{
                background: calMetric === key ? `${rgba}0.12)` : '#111',
                color:      calMetric === key ? color : '#6B6560',
                border:     `1px solid ${calMetric === key ? `${rgba}0.3)` : '#1C1C1C'}`,
              }}>
              <Icon size={10} />
              {label}
            </button>
        ...
        {calView === 'month' ? (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px' }}>
            {calDays.map((dateStr, idx) => {
              ...
              const val  = log ? (calMetric === 'calls' ? log.calls : log.appointments) : 0
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '6px' }}>
            {calDays.map(dateStr => {
              ...
              const val    = calMetric === 'calls' ? calls : appts
              const isToday  = dateStr === today
              const isFuture = dateStr > today
